  ...
  methods: {
    ...
    submitAdd(formItem) {
      ...
      this.$refs[formItem].validate((valid) => {
        if (valid) {
          this.http.apiPost(this.$urlList.enuserattr.create, paramData).then((res) => {
    ...
    submitedit(formItem) {
      ...
      this.$refs[formItem].validate((valid) => {
        if (valid) {
          this.http.apiPost(this.$urlList.enuserattr.update, paramData).then((res) => {
            if (res.code == '00000001') {
              this.$message.error(res.msg);
              this.$emit('update:loadingmsg', false);
            } else {
