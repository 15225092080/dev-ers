  ...
  <div>
    ...
    >
      <span slot="title" class="clearfix">
        <span class="dialog-title">{{ paramBack.dialogTitle + paramBack.title  }}</span>
...
export default {
  ...
  methods: {
    //关闭方法
    close() {
      this.$parent.visibleBack = false;
      this.$parent.$refs.dataTable.getTableData();
    },
    //获取表格数据
    getListData() {
      this.tableData = this.paramBack.tableData;
    }
  },
  created() {
    // 加载数据
