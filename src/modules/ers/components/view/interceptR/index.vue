  ...
  <div class="querypage">
    ...
    <el-card class="paddingtop">
      <div slot="header"
           class="clearfix">
        <span>
          <i class="DHCiconfont">&#xe66a;</i>评级上报拦截数据文件导入信息列表
        </span>
        <ul class="icongroup">
          <!-- <li>
            <el-button type="primary"
                       @click="downLoad">
              <i class="DHCiconfont">&#xe6c0;</i>上报拦截数据文件模板和说明下载
            </el-button>
          </li> -->
          <li>
...
export default {
  ...
  inject: ['reload'],
  data() {
    let uploadUrl = process.env.API_ROOT_ERS + this.$urlList.interceptR.upload;
    return {
      // 弹框相关
      paramDetail: {
        title: '',
        dialogTitle: '评级不上报拦截操作历史记录',
      },
      // 弹框相关
      paramEdit: {
        title: '',
      ...
      detailUrl: {
        ...
        url: this.$urlList.interceptR.getValUrl,
        delUrl: this.$urlList.interceptR.delete,
        paramDataId: 'id', // 字段名
        id: 'id', // 获取表格数据的字段名
        resId: '', // 获取的id
      },
  ...
  methods: {
    ...
    // downLoad() {
    //   this.common.downLoad(this.$urlList.interceptR.downloadModel, {});
    // },
    // rowClickFun(row) {
    //   this.visible = true;
