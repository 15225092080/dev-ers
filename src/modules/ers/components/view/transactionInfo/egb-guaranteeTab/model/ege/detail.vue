...
export default {
  ...
  data() {
    return {
      ...
      // 表单赋值接口（编辑，详情用的较多）
      findById: {
        url: this.$urlList.ege.getValUrl, // 接口
  ...
  methods: {
    eventChange(obj) {
      let _this = this; // 绑定指向
      _this.common.onselect('infoIdType', obj, function () { // 监听下拉框事件 参数为绑定的下拉字段、change事件返回、回调函数
    ...
    init() {
      this.arrList = [
        [
          {type: 'select', name: 'infoIdType', value: '', param: 'E_InfoIDType', label: '身份类别', span: 12, disabled: true, eventChange: true
          },
        ...
        [
          {type: 'select', name: 'arlpType', value: '', param: 'E_ArlpType_EG', label: '还款责任人类型', span: 12, disabled: true
          },
          {type: 'input', name: 'arlpAmt', value: '', label: '还款责任金额', span: 12, disabled: true
          }
        ],
        [
          {type: 'select', name: 'guarType', value: '', param: 'E_WartySign_EG', label: '联保标志', span: 12, disabled: true
          },
          {type: 'input', name: 'guarNum', value: '', label: '保证合同编号', span: 12, disabled: true
          }
        ],
        [
          ...
          {type: 'input', name: 'rptDate', value: '', label: '信息报告日期', span: 12, disabled: true}
        ],
        [
