  ...
  <div class="querypage">
    ...
    <el-card class="paddingtop">
      <div slot="header" class="clearfix">
        <span>
          <i class="DHCiconfont">&#xe66a;</i>担保信息记录列表
        </span>
        <ul class="icongroup">
          <li>
            <el-button type="primary" plain @click="addModal" v-permission="['crse:business:egb:create']">
              <i class="DHCiconfont">&#xe6c3;</i>创建
            </el-button>
          </li>
          <li>
            <el-button type="primary" @click="updateModal" v-permission="['crse:business:egb:update']">
              <i class="DHCiconfont">&#xe668;</i>编辑
            </el-button>
          </li>
          <li>
            <el-button type="danger" @click="deleteFun" v-permission="['crse:business:egb:delete']">
...
export default {
  ...
  components: {},
  inject: ['reload'],
  name:'egb-guaranteeTab',
  data() {
    return {
      detailUrl: {
        isTime: 'search_GTE_LTE_bussDate_DATE_RANGE',
        delUrl: this.$urlList.egb['delete'],
        paramDataId: 'id', // 字段名
        id: 'id', // 获取表格数据的字段名
        resId: '' // 获取的id
      },
      ...
      arrList: [
        [
          ...
          {type: 'select', name: 'search_EQ_acctType', value: '', param: 'E_AcctType_EG', label: '账户类型', span: 6},
  ...
  methods: {
    addModal() { // 暂时写到这里，日后新增编辑详情这种跳转或者弹出可能做成可配置
      ...
      this.$router.push({path: '/egb-guarantee-edit'});
    },
    updateModal(row) {
    ...
    showdetail(row) {
      ...
      this.$router.push({path: '/egb-guarantee-detail',
        query: {
          bussNum: row.bussNum
        }});
    }
