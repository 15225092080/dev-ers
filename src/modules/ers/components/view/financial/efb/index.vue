  ...
  <div class="querypage">
    ...
    <el-card class="paddingtop">
      <div slot="header" class="clearfix">
        <span>
          <i class="DHCiconfont">&#xe6cf;</i>02版企业资产负债列表
        </span>
        <ul class="icongroup">
          <li>
            <el-button type="primary" plain @click="addModal" v-permission="['crse:financial:efb:create']">
              <i class="DHCiconfont">&#xe6c3;</i>创建
            </el-button>
          </li>
          <li>
            <el-button type="primary" @click="updateModal" v-permission="['crse:financial:efb:update']">
              <i class="DHCiconfont">&#xe668;</i>编辑
            </el-button>
          </li>
          <li>
            <el-button type="danger" @click="deleteFun" v-permission="['crse:financial:efb:delete']">
...
export default {
  ...
  components: {},
  inject: ['reload'],
  name:'financial-efb-list',
  data() {
    return {
      detailUrl: {
        isTime: 'search_GTE_LTE_bussDate_DATE_RANGE',
        url: this.$urlList.efb.getValUrl,
        delUrl: this.$urlList.efb.delUrl,
        paramDataId: 'id', // 字段名
        id: 'id', // 获取表格数据的字段名
        resId: '' // 获取的id
      },
  ...
  methods: {
    addModal() { // 暂时写到这里，日后新增编辑详情这种跳转或者弹出可能做成可配置
      ...
      this.$router.push({path: '/financial-efb-edit'});
    },
    updateModal(row) {
    ...
    showdetail(row) {
      this.$router.push({path: '/financial-efb-detail',
        query: {
          id: row.id
        }});
    }
