  ...
  <div class="querypage">
    ...
    <el-card class="paddingtop">
      <div slot="header"
           class="clearfix">
        <span>
          <i class="DHCiconfont">&#xe6cf;</i>07版企业资产负债列表
        </span>
        <ul class="icongroup">
          <li>
...
export default {
  ...
  components: {},
  inject: ['reload'],
  name: 'financial-efc-list',
  data() {
    return {
      detailUrl: {
        isTime: 'search_GTE_LTE_bussDate_DATE_RANGE',
        url: this.$urlList.efc.getValUrl,
        delUrl: this.$urlList.efc.delUrl,
        paramDataId: 'id', // 字段名
        id: 'id', // 获取表格数据的字段名
        resId: '', // 获取的id
      },
      ...
      arrList: [
        ...
        [
          ...
          {
            type: 'select',
            name: 'search_EQ_sheetType',
            value: '',
            param: 'E_ReportFormType',
            label: '报表类型',
            span: 8,
          },
        ],
  ...
  methods: {
    addModal() {
      ...
      this.$router.push({ path: '/financial-efc-edit' });
    },
    updateModal(row) {
      if (
