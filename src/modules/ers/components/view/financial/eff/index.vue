  ...
  <div class="querypage">
    ...
    <el-card class="paddingtop">
      <div slot="header" class="clearfix">
        <span>
          <i class="DHCiconfont">&#xe6cf;</i>07版现金流量表
        </span>
        <ul class="icongroup">
          <li>
            <el-button type="primary" plain @click="addModal" v-permission="['crse:financial:eff:create']">
              <i class="DHCiconfont">&#xe6c3;</i>创建
            </el-button>
          </li>
          <li>
            <el-button type="primary" @click="updateModal" v-permission="['crse:financial:eff:update']">
              <i class="DHCiconfont">&#xe668;</i>编辑
            </el-button>
          </li>
          <li>
            <el-button type="danger" @click="deleteFun" v-permission="['crse:financial:eff:delete']">
...
export default {
  ...
  components: {},
  inject: ['reload'],
  name:'financial-eff-list',
  data() {
    return {
      detailUrl: {
        isTime: 'search_GTE_LTE_bussDate_DATE_RANGE',
        url: this.$urlList.eff.getValUrl,
        delUrl: this.$urlList.eff.delUrl,
        paramDataId: 'id', // 字段名
        id: 'id', // 获取表格数据的字段名
        resId: '' // 获取的id
      },
  ...
  methods: {
    addModal() { // 暂时写到这里，日后新增编辑详情这种跳转或者弹出可能做成可配置
      ...
      this.$router.push({path: '/financial-eff-edit'});
    },
    updateModal(row) {
    ...
    showdetail(row) {
      this.$router.push({path: '/financial-eff-detail',
        query: {
          id: row.id
        }});
    }
