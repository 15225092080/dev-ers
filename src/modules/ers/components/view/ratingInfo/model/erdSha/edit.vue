  ...
  methods: {
    eventChange(obj) {
      ...
      _this.common.onselect('sharhodIdType', obj, function () {
        ...
        _this.ipType = obj.e;
        let sharhodIdType = _this.$refs.formItem.formItem.sharhodIdType;
        // 监听下拉框事件 参数为绑定的下拉字段、change事件返回、回调函数
        _this.getSystemId(param, sharhodIdType);
      });
      _this.common.onselect('sharhodCertIdType', obj, function () {
        // 监听下拉框事件 参数为绑定的下拉字段、change事件返回、回调函数
        // 身份类型
        if (_this.ipType == 1) {
          if (obj.e == 10) {
    ...
    async getSystemId(param, sharhodIdType) {
    ...
    callbackFun(formItem) {
      ...
      if (param.invRatio) {
        if (typeof param.invRatio == 'number' || 'string') {
          param.invRatio = Number(param.invRatio).toFixed(2);
        }
      }
      this.callback(formItem, 'erdSha', param);
    },
    init() {
          ...
          [
            ...
            },
            {
              ...
              rule: [
                ...
                {
                  required: true,
                  validator: this.validator.invRatio,
                  trigger: 'blur',
                },
              ],
            },
