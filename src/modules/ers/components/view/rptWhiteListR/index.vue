  ...
  <div class="querypage">
    ...
    <el-card class="paddingtop">
      <div slot="header"
           class="clearfix">
        <span>
          <i class="DHCiconfont">&#xe66a;</i>评级上报白名单数据文件导入信息列表
        </span>
        <ul class="icongroup">
          <!-- <li>
...
export default {
  ...
  inject: ['reload'],
  data() {
    let uploadUrl = process.env.API_ROOT_ERS + this.$urlList.rptWhiteListR.upload;
    return {
      // 弹框相关
      paramDetail: {
        title: '',
        dialogTitle: '评级不上报白名单操作历史记录',
      },
      // 弹框相关
      paramEdit: {
        title: '',
        dialogTitle: '评级不上报白名单',
      },
      uploadData: {
        // 按钮文字
        btnText: '评级上报白名单数据文件上传',
        // url
        uploadUrl: uploadUrl,
        // 文件大小
        fileSize: 2,
      ...
      detailUrl: {
        ...
        url: this.$urlList.rptWhiteListR.getValUrl,
        delUrl: this.$urlList.rptWhiteListR.delete,
        paramDataId: 'id', // 字段名
        id: 'id', // 获取表格数据的字段名
        resId: '', // 获取的id
      },
  ...
  methods: {
    ...
    // downLoad() {
    //   this.common.downLoad(this.$urlList.rptWhiteListR.downloadModel, {});
    // },
    // rowClickFun(row) {
    //   this.visible = true;
