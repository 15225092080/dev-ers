...
export default {
  ...
  methods: {
    ...
    eventChange(obj) {
      ...
      _this.common.onselect('assoEntCertType', obj, function () { // 监听下拉框事件 参数为绑定的下拉字段、change事件返回、回调函数
        if (obj.e == 10) {
          _this.common.Control(_this.arrList, {
            'assoEntCertNum': {
              rule: [
    ...
    init() {
      ...
      if (this.$store.getters.operatState == 'add') {
        // 根据弹框状态改变提交url
        this.url = this.$urlList.ecc.creUrl;
        // 页面表单
        this.arrList = [
          [
            {type: 'input',
              ...
              rule: [
                ...
                // {required: true, validator: this.validator.validatorCustId, trigger: 'blur'}
        ...
        // 根据弹框状态改变提交url
        this.url = this.$urlList.ecc.updUrl;
        // 页面表单
        this.arrList = [
          [
            {type: 'input',
