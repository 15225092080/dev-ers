  ...
  <div class="querypage">
    ...
    <el-card class="paddingtop">
      <div slot="header"
           class="clearfix">
        <span>
          <i class="DHCiconfont">&#xe66a;</i>信贷上报拦截数据文件导入信息列表
        </span>
        <ul class="icongroup">
          <!-- <li>
...
export default {
  ...
  inject: ['reload'],
  data() {
    let uploadUrl = process.env.API_ROOT_ERS + this.$urlList.interceptA.upload;
    return {
      // 弹框相关
      paramDetail: {
        title: '',
        dialogTitle: '信贷不上报拦截操作历史记录',
      },
      // 弹框相关
      paramEdit: {
        title: '',
      ...
      detailUrl: {
        ...
        url: this.$urlList.interceptA.getValUrl,
        delUrl: this.$urlList.interceptA.delete,
        paramDataId: 'id', // 字段名
        id: 'id', // 获取表格数据的字段名
        resId: '', // 获取的id
      },
  ...
  methods: {
    ...
    // downLoad() {
    //   this.common.downLoad(this.$urlList.interceptA.downloadModel, {});
    // },
    // rowClickFun(row) {
    //   this.visible = true;
